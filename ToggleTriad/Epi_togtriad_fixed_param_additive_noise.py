...
N_INIT = 25 #no. of random initial conditions
TIME_STEPS = 30000
dt = 0.1
sqrt_dt = np.sqrt(dt) # pre computing for efficiency
noise_factor = 4 #noise strength
...
def simulate_triad(N_INIT, TIME_STEPS, dt):
    ...
    for it in range(N_INIT):
        ...
        for t in range(TIME_STEPS-1):
            ...
            # Additive noise (added at each step instead of at intervals as in the paper), ensuring non negative values using max(0, ...)
            A[t+1] = max(0, A[t] + dt * (g_A * inhib_A - k_A * A[t]) + noise_factor * sqrt_dt * np.random.randn())
            B[t+1] = max(0, B[t] + dt * (g_B * inhib_B - k_B * B[t]) + noise_factor * sqrt_dt * np.random.randn())
            C[t+1] = max(0, C[t] + dt * (g_C * inhib_C - k_C * C[t]) + noise_factor * sqrt_dt * np.random.randn())
